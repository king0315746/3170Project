...
BEGIN
    ...
    -- 检查Shipping Status是否�?"N"，如果不是，则返回错误信�?
    IF (v_shipping_status != 'N' OR v_quantity <= 0) THEN
        SELECT 'The books in the order are shipped' AS error_message;
        -- 返回错误消息
    ELSE
        -- 更新订单中书的Quantity和书的剩余数�?
        UPDATE ordering od
        Join orders o on o.order_id = od.order_id
        SET od.quantity = od.quantity - delete_quantity, -- 替换为要删除的副本数�?
            o.order_date = CURDATE() -- 更新为当前系统日�?
        WHERE o.order_id = v_order_id; -- 替换为实际的订单 ID

        UPDATE book
        SET no_of_copies = no_of_copies + delete_quantity -- 替换为要删除的副本数�?
